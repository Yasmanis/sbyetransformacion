<template>
    <q-table
        wrap-cells
        flat
        :filter="filter"
        :columns="columns"
        :rows="billingsInformation"
        :pagination="{ rowsPerPage: 0 }"
        v-model:selected="selected"
        selection="multiple"
        hide-pagination
        row-key="id"
    >
        <template v-slot:top>
            <q-toolbar class="no-padding">
                <div class="col col-auto no-padding">
                    <q-input
                        dense
                        clearable
                        debounce="300"
                        v-model="filter"
                        placeholder="filtrar"
                    >
                        <template v-slot:append>
                            <q-icon name="search" />
                        </template>
                    </q-input>
                </div>
                <q-space />
                <div class="col-auto">
                    <form-billing-information />
                    <delete-component
                        url="/admin/users/billing-information"
                        :objects="selected"
                        @deleted="selected = []"
                        v-if="selected.length > 0"
                    /></div
            ></q-toolbar>
        </template>

        <template v-slot:header-selection="scope">
            <q-checkbox v-model="scope.selected" size="sm" />
        </template>

        <template v-slot:body-selection="scope">
            <q-checkbox v-model="scope.selected" size="sm" />
        </template>

        <template v-slot:body-cell-predetermined="props">
            <td class="text-center">
                <q-chip
                    dense
                    size="sm"
                    style="max-width: min-content"
                    :color="props.value ? 'black' : 'blue-2'"
                    :text-color="props.value ? 'white' : 'black'"
                    :icon="props.value ? 'check' : 'error'"
                    :label="props.value ? 'Si' : 'No'"
                />
            </td>
        </template>
        <template v-slot:body-cell-actions="props">
            <td class="text-center" width="120px">
                <form-billing-information :object="props.row" />
                <btn-confirm-component
                    :disabled="props.row.predetermined"
                    tooltips="establecer como predeterminado"
                    @click="
                        router.post(
                            `/admin/users/billing-information/predetermined/${props.row.id}`,
                        )
                    "
                />
                <delete-component
                    url="/admin/users/billing-information"
                    :objects="[props.row]"
                />
            </td> </template
    ></q-table>
</template>

<script setup>
import { ref } from "vue";
import { router } from "@inertiajs/vue3";
import FormBillingInformation from "../../modules/shopping/userdata/FormBillingInformation.vue";
import DeleteComponent from "../../table/actions/DeleteComponent.vue";
import BtnConfirmComponent from "../../btn/BtnConfirmComponent.vue";
import { billingsInformation } from "../../../services/shopping";

defineOptions({
    name: "ListBillingInformation",
});

const filter = ref("");

const selected = ref([]);

const columns = [
    {
        name: "full_name",
        field: "full_name",
        label: "nombre",
        align: "left",
    },
    {
        name: "nif_cif",
        field: "nif_cif",
        label: "nif/cif",
        align: "left",
    },
    {
        name: "country_str",
        field: "country_str",
        label: "pais",
        align: "left",
    },
    {
        name: "province",
        field: "province",
        label: "provincia",
        align: "left",
    },
    {
        name: "road",
        field: "road",
        label: "via",
        align: "left",
    },
    {
        name: "address",
        field: "address",
        label: "direccion",
        align: "left",
    },
    {
        name: "postal_code",
        field: "postal_code",
        label: "c. postal",
        align: "left",
    },
    {
        name: "predetermined",
        field: "predetermined",
        label: "predeterminado",
        align: "center",
    },
    {
        name: "actions",
        field: "actions",
        label: "",
        align: "center",
    },
];
</script>
