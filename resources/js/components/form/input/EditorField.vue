<template>
    <ckeditor v-model="model" :editor="editorProps.editor" :config="editorProps.config" @ready="onReady"
        @input="onEditorInput" />
</template>
...
const editorProps = ref({
    ...
    config: {
        ...
        toolbar: [
            ...
            'outdent',
            'indent', '|',
            'fontSize', 'fontFamily', 'fontColor', 'fontBackgroundColor', 'alignment', 'horizontalLine', '|', 'save-btn'
        ],
        heading: {
            options: [
                {
...
const onReady = (editor) => {
    editor.editing.view.change((writer) => {
        writer.setStyle("height", `${screen.value.height - 115}px`, editor.editing.view.document.getRoot()
        )
    });
};
