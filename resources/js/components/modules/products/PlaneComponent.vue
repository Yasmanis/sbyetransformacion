<template>
    <checkbox-field v-if="inForm" :name="name" :label="label" />
    <q-btn-component
        icon="attach_money"
        tooltips="planes"
...
import NumberField from "../../form/input/NumberField.vue";
...
const onShow = () => {
    ...
    const actives = props.product.planes ?? [];
    actives.forEach((p) => {
...
const saveProductPlan = () => {
    ...
    if (props.product) {
        ...
        emits("update", data.length > 0 ? data : null);
    }
};
</script>
