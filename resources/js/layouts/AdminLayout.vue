<template>
    <q-layout view="lHh Lpr lFf">
        <q-header>
            <q-toolbar :style="{ background: Dark.isActive ? '#1d1d1d' : '#407492' }">
                ...
                <q-btn-component icon="reply_all" tooltips="volver a" color="white" class="q-mr-sm">
                    <q-menu style="min-width: 300px" transition-show="scale" transition-hide="scale">
                        <q-list separator>
                ...
                <q-btn-component icon="mdi-bell-outline" color="white" class="q-mr-sm" v-if="user.notifications.length > 0">
                    <q-badge :color="Dark.isActive ? 'primary' : 'black'" floating class="fa-beat">{{
                        user.notifications.length
                    }}</q-badge>
                    <q-menu style="min-width: 300px" transition-show="scale" transition-hide="scale">
                        <notifications-component :messages="user.notifications" />
                    </q-menu>
                </q-btn-component>

                <q-btn-component color="white" class="q-mr-sm">
                    <q-avatar size="md" class="shadow-10"><img
                            :src="`${user.avatar ? user.avatar : `${$page.props.public_path}images/icon/profile.svg`}`" /></q-avatar>
                    <q-menu style="width: 240px" transition-show="scale" transition-hide="scale">
                        <q-card>
                            <q-card-section class="q-pa-none">
                                <q-list dense>
                                    <q-item>
                                        <q-item-section avatar>
                                            <q-avatar size="lg" class="shadow-10"><img
                                                    :src="`${user.avatar ? user.avatar : `${$page.props.public_path}images/icon/profile.svg`}`" /></q-avatar>
                                        </q-item-section>
                                    ...
                                    <q-separator />
                                    <q-item clickable @click="router.get('/auth/profile')">
                                        <q-item-section avatar>
                                            <q-icon name="mdi-account-outline" /> </q-item-section><q-item-section>
                                            <q-item-label>
                                                perfil
                                            </q-item-label>
                                        </q-item-section>
                                    ...
                                    <q-item clickable @click="showPasswordDialog = true">
                                        <q-item-section avatar>
                                            <q-icon name="mdi-key-outline" /> </q-item-section><q-item-section>
                                            <q-item-label>
                                                cambiar contraseña
                                            </q-item-label>
                                        </q-item-section>
        ...
        <q-drawer :class="Dark.isActive ? '' : 'bg-primary text-white'" v-model="leftDrawerOpen" show-if-above :mini="true"
            v-if="mini">
        ...
        <q-footer>
            <q-toolbar :class="Dark.isActive ? 'bg-dark' : ''">
                <q-toolbar-title>
                    <span class="text-body1 text-white">©2024 sbye salud s.l.
                    </span></q-toolbar-title>
                <q-btn-component size="md" href="https://www.facebook.com/profile.php?id=61563937152210" target="blank"
                    color="white" padding="8px">
                    <q-icon name="fab fa-facebook-f" size="xs" />
                </q-btn-component>
                <q-btn-component size="md" style="margin-left: 10px; margin-right: 10px"
                    href="https://www.youtube.com/@sbyetransformacion" target="blank" color="white" padding="8px">
                    <q-icon name="fab fa-youtube" size="xs" />
                </q-btn-component>
                <q-btn-component size="md" style="margin-right: 10px" href="https://www.instagram.com/sbyetransformacion/"
                    target="blank" color="white" padding="8px">
                    <q-icon name="fab fa-instagram" size="xs" />
                </q-btn-component>
                <q-btn-component size="md" href="https://www.tiktok.com/@sbyetransformacion" target="blank" color="white"
                    padding="8px">
    ...
    <q-dialog v-model="showPasswordDialog" persistent>
        <q-card class="scroll">
            <dialog-header-component icon="mdi-account-key-outline" title="cambiar mi contraseña" closable />
            <q-card-section class="col q-pt-none">
                <q-form class="q-gutter-sm q-mt-sm" ref="formPassword" greedy>
                    <password-field label="nueva contraseña" name="password" :othersProps="{ required: true }" oldPassword
                        @update="onUpdateField" @confirm="onUpdateField" @old-password="onUpdateField" />
                </q-form>
            </q-card-section>
            <q-separator />
            <q-card-actions align="right">
                <btn-save-component @click="changePassword" />
                <btn-cancel-component :cancel="true" @click="showPasswordDialog = false" />
            </q-card-actions>
        </q-card>
    </q-dialog>
</template>
